...
Free services used (no paid keys required):
  ...
  - Hugging Face     : text-to-image, free with a read token (no card)
  - Pollinations.ai : text-to-image, secondary attempt, no key needed
...
# ---------------------------------------------------------------------------
# Step 2: Scene images — Hugging Face first (reliable), Pollinations second,
# local placeholder as last resort
# ---------------------------------------------------------------------------
async def _generate_image_hf(image_prompt: str, out_path: Path) -> bool:
    if not HF_API_TOKEN:
        return False
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for attempt in range(2):
            try:
                resp = await client.post(url, headers=headers, json={"inputs": image_prompt})
...
async def _generate_image_pollinations(image_prompt: str, out_path: Path,
                                        width: int, height: int) -> bool:
    encoded = urllib.parse.quote(image_prompt)
    url = f"https://image.pollinations.ai/prompt/{encoded}?width={width}&height={height}&nologo=true"
    headers = {"User-Agent": "Mozilla/5.0 (compatible; AILabStudent/1.0)"}
    try:
...
async def generate_image(image_prompt: str, out_path: Path, width: int, height: int):
    if await _generate_image_hf(image_prompt, out_path):
        return
    if await _generate_image_pollinations(image_prompt, out_path, width, height):
        return
    subprocess.run(
        ["ffmpeg", "-y", "-f", "lavfi",
         "-i", f"color=c=0x203350:s={width}x{height}",
...
async def run_pipeline(job_id: str, req: CreateJobRequest):
    ...
    try:
        ...
        img_paths = [job_dir / f"scene_{i}.jpg" for i in range(len(scenes))]
        audio_paths = [job_dir / f"scene_{i}.mp3" for i in range(len(scenes))]

        image_tasks = [
            generate_image(scene["image_prompt"], img_paths[i], width, height)
            for i, scene in enumerate(scenes)
        ]
        voice_tasks = [
            generate_voice(scene["narration"], audio_paths[i], voice)
            for i, scene in enumerate(scenes)
        ]
        await asyncio.gather(*image_tasks)
        JOBS[job_id] = {"status": "generating_assets", "progress": 45}
        durations = await asyncio.gather(*voice_tasks)
        JOBS[job_id] = {"status": "generating_assets", "progress": 60}

        clip_paths = []
        for i, scene in enumerate(scenes):
            clip_path = job_dir / f"clip_{i}.mp4"
            build_scene_clip(img_paths[i], audio_paths[i], scene["narration"],
                              durations[i], clip_path, width, height)
            clip_paths.append(clip_path)
            JOBS[job_id] = {
                "status": "generating_assets",
                "progress": 60 + int(30 * (i + 1) / len(scenes)),
            }
