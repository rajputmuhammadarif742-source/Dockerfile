...
async def generate_scenes(topic: str, num_scenes: int) -> list[dict]:
    if not GROQ_API_KEY:
        # Fallback so the app still works with zero setup (lower quality, no LLM)
        return [
            {
...
# ---------------------------------------------------------------------------
# Step 2: Scene images via Pollinations.ai (free, no key)
# ---------------------------------------------------------------------------
async def generate_image(image_prompt: str, out_path: Path, width: int, height: int):
    ...
    # Image service is down after retries — fall back to a plain placeholder
    # so the video still gets made instead of the whole job failing.
    subprocess.run(
        ["ffmpeg", "-y", "-f", "lavfi",
